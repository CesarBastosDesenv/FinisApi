...
{
    private ILogger<MovimentacaoController> _logger;
    private IMovimentacaoService _movimentacaoService;

    public MovimentacaoController(ILogger<MovimentacaoController> logger, IMovimentacaoService movimentacaoService)
    {
        _logger = logger;
    ...
    public async Task<ActionResult> Get([FromQuery] PaginationParams paginationParams)
    {
        try
        {
            var result = await _movimentacaoService.GetAllAsync(paginationParams.PageNumber, paginationParams.PageSize);
    ...
    public async Task<ActionResult> Get(int Id)
    {
        try
        {
            var result = await _movimentacaoService.BuscaMovimentacaoId(Id);
            return Ok(result);
        }
        catch (Exception ex)
        {
    ...
    [HttpPost]
    public async Task<ActionResult> Add(MovimentacaoCadastro args)
    {
        try
        {
            if (!ModelState.IsValid) return Ok(new ResultViewModel(args, ModelState));
            var result = await _movimentacaoService.AddAsync(args);
            return Ok(result);
        }
        catch (Exception ex)
        {
