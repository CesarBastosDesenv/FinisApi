...
{
    private ILogger<ContaController> _logger;
    ...
    public async Task<ActionResult> Get([FromQuery] PaginationParams paginationParams)
    {
        try
        {
            var result = await _contaService.GetAllAsync(paginationParams.PageNumber, paginationParams.PageSize);
    ...
    public async Task<ActionResult> Get(int Id)
    {
        try
        {
            var result = await _contaService.BuscaContaId(Id);
            return Ok(result);
        }
        catch (Exception ex)
        {
    ...
    [HttpPost]
    public async Task<ActionResult> Add(ContaCadastro args)
    {
        try
        {
            if (!ModelState.IsValid) return Ok(new ResultViewModel(args, ModelState));
            var result = await _contaService.AddAsync(args);
            return Ok(result);
        }
        catch (Exception ex)
        {
